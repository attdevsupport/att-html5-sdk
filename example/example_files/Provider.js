/**
 * Att.Provider exposes methods to access the AT&T APIs from a Sencha Touch-based
 * application. When a method is called on the client side, a request is made to
 * the server side. The server will validate the request and then make the
 * appropriate call to the AT&T API.
 *
...
This method is required and supported only for applications attempting to consume and access the My Messages (MIM (**Beta**) ), and Message On Behalf Of (MOBO (**Beta**) ).
