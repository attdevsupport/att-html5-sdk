		...
		try
		{
			...
			for (int i = 0; i < Image_List().size(); i++) {
				...
				try
				{
					...
					driver.get(url);
					waitLonger.until(ExpectedConditions.visibilityOfElementLocated(By.name("address")));
					testResult.info("setting Address");
					WebElement address = driver.findElement(By.name("address"));
					address.clear();
					address.sendKeys(number);
					
					testResult.info("Selecting image " + imageName + " from listbox");
					...
					String DeliveryAttempt = driver.findElement(By.id("serverResponse")).getAttribute("innerText");
					if(DeliveryAttempt.contains("messageId"))
					{
						//Close Response
						testResult.setAction("Wait for close button");
						wait.until(ExpectedConditions.elementToBeClickable(By.id("btnCloseResponse")));
						...
						testResult.setAction("Wait For Visiblity of ServerResponse");
						wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("serverResponse")));
						result = driver.findElement(By.id("resultsHeader")).getText();
