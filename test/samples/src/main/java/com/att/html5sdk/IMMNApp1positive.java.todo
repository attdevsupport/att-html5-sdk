...
public class IMMNApp1positive {
	...
	public void Execute(String url, String address, String message, String subject, String attachment, String helloPg, String sendMessageBtn) throws InterruptedException, IOException{
		...
		try{
		// Send Message
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id(address))).sendKeys(global.phoneNumber);
		...
		if (driver.findElement(By.id("ext-element-96")).getText().contains("Success: true"))
	     	global.CreateFile("\r\n" + "PASSED" + "\r\n");
	     else
	    	global.CreateFile("\r\n" + "FAILED" + "\r\n");
		
		if (url.contains("Java-SDK/IMMN"))
	    	 global.CreateFile("***** IMMN JAVA Send Message : " + "\r\n" + driver.findElement(By.id("ext-element-96")).getText() + "\r\n\r\n");
	     else if (url.contains("Ruby-SDK/IMMN"))
	    	 global.CreateFile("***** IMMN RUBY Send Message: " +  "\r\n" + driver.findElement(By.id("ext-element-96")).getText() + "\r\n\r\n");   	  
	     else 
	    	 global.CreateFile("***** IMMN PHP Send Message: " + "\r\n" + driver.findElement(By.id("ext-element-96")).getText() + "\r\n\r\n");    
