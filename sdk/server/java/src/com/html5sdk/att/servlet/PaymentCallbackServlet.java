 ...
 * this URL with a 'code' parameter. This is exchanged for an access token,
 * which can be used in any future calls to the AT&T APIs
 * 
 * @class com.html5sdk.att.servlet.PaymentCallbackServlet
 * 
 */
public class PaymentCallbackServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request,
        ...
        // Find the TransactionAuthCode or SubscriptionAuthCode on the query
        // string.
        String transactionAuthCode = request
                .getParameter(AttConstants.TransactionAuthCode);

        if (transactionAuthCode == null) {
            transactionAuthCode = request
                    .getParameter(AttConstants.SubscriptionAuthCode);
        }
        ...
        try {
            if (transactionAuthCode != null) {
                ...
                // If the payment fails to complete the app will have one set of
                // query params
                if ("false".equals(request.getParameter("success"))) {
                    results.put("error_reason",
                            request.getParameter("faultCode"));
                    results.put("error_description",
                            request.getParameter("faultDescription"));
                } else {
                    // If the user clicks cancel then we get a different set of
                    // errors.
