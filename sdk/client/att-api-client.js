...
AttApiClient = (function () {
    ...
    return {
        ...
        /**
         * The Push Notification API allows apps to subscribe to and receive push notifications when messages are added,
         * updated or deleted in a user's message inbox. This specification provides details about the framework and
         * mechanism for subscribing and receiving Push Notifications from compatible AT&T APIs. This specification
         * also provides a minimum level of detail for the events generated by participating APIs. For more detailed
         * information on those events, see the sections specific to the AT&T APIs generating the events.
         * @class AttApiClient.Notification
         * @singleton
         */
        Notification: {
            /**
             * Gets the Notification Channel details for the channel created by the app server.
             * Requires that enableUnsafeOperations is set to true on the app server.
            ...
            /**
             * Creates a notification subscription for the In-App Messaging MIM scope.
             * @param {Object} data Parameters of the call.
             * @param {Object} data.subscription Object containing the subscription parameters.
             * @param {String[]} data.subscription.events List of notification events. Values for this parameter are:
             * <ul><li>TEXT - all events relating to SMS</li>
             * <li>MMS - all events relating to MMS</li></ul>
             * Defaults to all events if omitted
             * @param {Number} data.subscription.expiresIn Lifetime of the subscription in seconds. Values for this parameter are:
             * <ul><li>0 means the subscription never expires</li>
             * <li>Value between 3600 (1 hour) and 31536000 (1 year)</li></ul>
             ...
             * @param {Object} subscription The parameters to be updated within the subscription.
            ...
            /**
             * Get the list of current notification events. The events will remain on the app server; use
             * deleteNotifications to remove the notifications.
             * @param {Object} data Parameters of the call.
            ...
            /**
             * Gets notifications from the app server and deletes them.
             * @param {Object} data Parameters of the call.
        ...
        SMS: {
             ...
             *
             * @param {Object} data An object that may contain the following properties:
             *   @param {String} data.addresses Wireless number of the recipient(s). A comma-separated string where each item is either a phone number (including area code) or a short code. A phone number should be preceded by 'tel:', and a short code should be preceded by 'short:'. The following address string illustrates both types: "tel:4258675309,short:123456".
             ...
             * <li><b>An error in the connection between the client and the SDK server:</b> 'info' is the jQuery object used to send the network request (jqXHR). This is structurally similar to a native XMLHttpRequest object; the 'status' property provides the HTTP status code, and the 'response' property may contain additional detail.</li>
             * <li><b>An error in either internal SDK server processing or network communication between the SDK server and the AT&T back-end services:</b> 'info' is a jqXHR as described above. The 'status' property will be a 4xx HTTP status code. The 'response' property is a JSON object whose 'error' property is a string describing the error.</li>
             * <li><b>An error from the AT&T back-end services:</b> 'info' is a jqXHR as described above. The 'response' property is a JSON object whose 'error' property is a structured JSON error object returned by the back end. Please refer to the online developer documentation for descriptions of possible fields in this error object; one example is shown below.
             * <pre>
             * {'error': {
             *     'RequestError': {
             *         'ServiceException': {
             ...
             * <ol>
             * <li><b>An error detected in client processing (typically, when missing parameters are detected):</b> 'info' is an array of strings, each an error description.</li>
             * <li><b>An error in the connection between the client and the SDK server:</b> 'info' is the jQuery object used to send the network request (jqXHR). This is structurally similar to a native XMLHttpRequest object; the 'status' property provides the HTTP status code, and the 'response' property may contain any additional detail.</li>
             * <li><b>An error in either internal SDK server processing or network communication between the SDK server and the AT&T back-end services:</b> 'info' is a jqXHR as described above. The 'status' property will be a 4xx HTTP status code. The 'response' property is a JSON object whose 'error' property is a string describing the error.</li>
             * <li><b>An error from the AT&T back-end services</b> 'info' is a jqXHR as described above. The 'response' property is a JSON object whose 'error' property is a structured JSON error object returned by the back-end. Please refer to the online developer documentation for descriptions of possible fields in this error object; one example is shown below.
             * <pre>
             * {'error': {
            ...
            /**
             * Gets a list of SMS messages sent to the app's short code
             *
             * @param {Object} data An object which may contain the following properties:
             *   @param {Number} data.shortcode The ShortCode/RegistrationId from which to receive messages.
             * @param {Function} success Success callback function.
             *   @param {Object} success.response A JSON object formatted as follows:
             * <pre>
             *       {
        ...
        MMS: {
             ...
             *   @param {String} params.addresses Wireless number of the recipient(s). A comma-separated string where each item is either a phone number (including area code) or a short code. A phone number should be preceded by 'tel:', and a short code should be preceded by 'short:'. The following address string illustrates both types: "tel:4258675309,short:123456".
             *   @param {String} params.message The text of the message to send.
             *   @param {String} params.fileId (optional) The name of the file on the server to be attached to the message.
             * @param (FormData) formData The attachments to be included with the MMS message. Pass null if there are no attachments.
             * @param {Function} success Success callback function.
             *   @param {Object} success.response A JSON object formatted as follows:
             * <pre>
             *       {
        ...
        InAppMessaging: {
             ...
             *   @param {String} data.addresses the message recipients. A comma-separated string where each item is either a phone number (including area code), a short code, or an email address. A phone number should be preceded by 'tel:', and a short code should be preceded by 'short:'. The following address string illustrates all three types is "demo@example.com,tel:4258675309,short:123456".
        ...
        util: {
            ...
            /**
             * Given an email, returns true or false if it is in a valid format.
            ...
            /**
             * Given a short code, returns true or false if it is in a valid format.
            ...
            /**
             * Given an address will determine if it is a valid phone, email or short code.
